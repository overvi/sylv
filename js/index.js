// Navigation Scroll Stick To Top
...
function languageHandler() {
  if (!language) {
    ...
    themeLightModeBtn.style.color = "black";
  } else {
    if (language === "en") {
      languageFaBtn.style.color = "black";
    } else {
      chooseFaLanguage(languageFaBtn);
    }
  }
}
...
function chooseFaLanguage(e) {
  ...
  elementFa.firstElementChild.style.transform = "translateX(-20px)";
  ...
  chooseLanguageBtn.firstElementChild.style.transform = "translateX(25px)";
...
function chooseEnLanguage(e) {
  e.currentTarget.firstElementChild.style.transform = "translateX(20px)";
  e.currentTarget.firstElementChild.style.color = "white";
...
function openSelectBoxHandler(event, id, selectBoxData = []) {
 ...
 if (!sliderBox.dataset.bound) {
  sliderBox.addEventListener("mousedown", (optionElement) => {
    ...
    if (input.multiple) {
      const existingIdx = selectBoxData.findIndex(item => item.li === liItem);
      ...
      if (liItem.classList.contains("selected")) {
        ...
        if (!liItem.dataset.explain) {
          if (existingIdx === -1) selectBoxData.push({ li: liItem, value: liItem.textContent });
        }
      }
...
Array.from(inpus).map((element) => {
  ...
  element?.addEventListener("blur", (element) => {
    element.currentTarget.placeholder = "Type...";
  });
});
